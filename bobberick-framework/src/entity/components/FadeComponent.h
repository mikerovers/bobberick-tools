...
{
public:
	FadeComponent(std::string textureID, int opacity, int fadeDelay, bool fadeIn, bool fadeOut);
	FadeComponent(std::string textureID, int opacity, int fadeDelay, int reverseDelay); // Will initiate both fadeIn and fadeOut as true.
	...
	int opacity; // The current opacity (0 = invisible, 255 = opaque).
	int fadeDelay; // The remaining delay in ticks before opacity changes by 1.
	int fadeDelayMax; // The maximum delay in ticks before opacity changes by 1.
}
